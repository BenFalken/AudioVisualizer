...
export class Assignment3 extends Scene {
    constructor() {
        ...
        this.materials = {
            ...
            planet4_mat: new Material(new defs.Phong_Shader(),
                { ambient: 0, diffusivity: 0.3, specularity: 1, color: hex_color("#ADD8E6") }),
            moon_mat: new Material(new Gouraud_Shader(),
                { ambient: 0, diffusivity: 1, specularity: 1, color: hex_color("#294e1a") })
        }
    ...
    display(context, program_state) {
        ...
        let sun_scale = 2 + Math.sin(0.9 * t);
        let sun_color = color(1, 0.5 + 0.5 * Math.sin(0.9 * t), 0.5 + 0.5 * Math.sin(0.9 * t), 1.00);

        program_state.lights = [new Light(vec4(0, 0, 0, 1), sun_color, 10 ** sun_scale)];

        sun_transform = sun_transform.times(Mat4.scale(sun_scale, sun_scale, sun_scale));
        sun_transform = sun_transform.times(Mat4.translation(0, 0, 0))
        this.shapes.sphere.draw(context, program_state, sun_transform, this.materials.sun.override(sun_color));
        ...
        let p1_angle = 0.9*t;
        p1_transform = p1_transform.times(Mat4.rotation(p1_angle, 0, 0, 1));
        p1_transform = p1_transform.times(Mat4.translation(5, 0, 0));
        this.shapes.planet1.draw(context, program_state, p1_transform, this.materials.planet1_mat);

        let p2_transform = Mat4.identity();
        let p2_angle = 0.8 * (t);
        p2_transform = p2_transform.times(Mat4.rotation(p2_angle, 0, 0, 1));
        p2_transform = p2_transform.times(Mat4.translation(8, 0, 0));
        if (Math.floor(t % 2) == 0) {
            this.shapes.planet2.draw(context, program_state, p2_transform, this.materials.planet2_phong);
        }
        else {
            this.shapes.planet2.draw(context, program_state, p2_transform, this.materials.planet2_gouraud);
        }

        let p3_transform = Mat4.identity();
        let p3_angle = 0.7 * (t);
        let p3_wobble =  Math.sin(0.85 * t);
        p3_transform = p3_transform.times(Mat4.rotation(p3_angle, 0, 0, 1));
        p3_transform = p3_transform.times(Mat4.translation(11, 0, 0));
        p3_transform = p3_transform.times(Mat4.rotation(p3_wobble, 1, 1, 0));
        this.shapes.planet3.draw(context, program_state, p3_transform, this.materials.planet3_mat);

        let ring_transform = p3_transform.times(Mat4.scale(4, 4, 1));
        this.shapes.torus2.draw(context, program_state, ring_transform, this.materials.ring_mat);

        let p4_transform = Mat4.identity();
        let p4_angle = 0.6 * t;
        p4_transform = p4_transform.times(Mat4.rotation(p4_angle, 0, 0, 1));
        p4_transform = p4_transform.times(Mat4.translation(14, 0, 0));
        this.shapes.planet4.draw(context, program_state, p4_transform, this.materials.planet4_mat);

        let moon_transform = p4_transform;
        let moon_orbit = t;
        moon_transform = moon_transform.times(Mat4.rotation(moon_orbit, 0, 0, 1.25));
        ...
        this.moon = moon_transform;//.times(Mat4.rotation(-moon_orbit, 0, 0, 1.25));
...
class Ring_Shader extends Shader {
    ...
    vertex_glsl_code() {
        ...
        return this.shared_glsl_code() + `
        attribute vec3 position; // position of vertex in object space
        uniform mat4 model_transform; // object space to world space
        uniform mat4 projection_camera_model_transform;
    ...
    fragment_glsl_code() {
        ...
        return this.shared_glsl_code() + `
        void main(){
            float factor = sin(distance(point_position,center) * 13.0);
            vec4 mixed_color = vec4(0.2, 0.2, 0.1, factor);
